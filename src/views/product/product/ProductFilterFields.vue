...
import type { ParamsProductOptions } from '@/types/product.type';
...
const categoryOptions = ref<OptionsValue[]>([]);
const unitOptions = ref<OptionsValue[]>([]);
...
const getFilterOptions = async (field: string): Promise<void> => {
  try {
    let params: ParamsProductOptions = {};
    if (field === 'brand') {
      params.brand = JSON.stringify(true);
    }
    if (field === 'category') {
      params.category = JSON.stringify(true);
    }
    if (field === 'unit') {
      params.unit = JSON.stringify(true);
    }
    const { data } = await ProductService.getProductOption(params);
    if (field === 'brand') {
      brandOptions.value = data.data.brand;
    }
    if (field === 'category') {
      categoryOptions.value = data.data.category;
    }
    if (field === 'unit') {
      unitOptions.value = data.data.unit;
    }
  } catch (error) {
  ...
  >
    ...
    <div class="!overflow-hidden">
      <MultiSelect
        ...
        @show="getFilterOptions('category')"
        controls
        label="Kategori"
