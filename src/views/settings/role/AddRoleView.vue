...
const createRole = async (): Promise<void> => {
  try {
    ...
    toast.add({
      ...
      summary: 'Gagal',
      detail: error
        ? (error as ApiError).response?.data?.message || 'Gagal menambah role'
        : 'Gagal menambah role',
      life: 3000,
    });
  }
...
<template>
  <Card>
    <form class="flex flex-col p-4 gap-3 text-xs" @submit.prevent="createRole">
