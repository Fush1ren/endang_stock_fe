...
const clearFields = (): void => {
  ...
  fromStoreModel.value = undefined;
  toStoreModel.value = undefined;
  productModel.value = [];
  quantityModel.value = [];
  ...
  if (productModel.value[index]) {
    if (isWarehouseModel.value === 'warehouse') {
      const stockItem = stockWarehouse.value.find(
        (item) => item.id === productModel.value[index].id,
      );
...
const getStock = async (): Promise<void> => {
  if (isWarehouseModel.value == 'store') {
    await getStoreStock();
  } else if (isWarehouseModel.value == 'warehouse') {
...
const getStoreStock = async (): Promise<void> => {
  try {
    if (!fromStoreModel.value) return;
    const { data } = await StockService.getStoreStockById(
      fromStoreModel.value.id,
    );
...
const createStockMutation = async (): Promise<void> => {
  try {
    ...
    await StockService.postStockMutation(stockMutationData);
    toast.add({
      severity: 'success',
      summary: 'Success',
      detail: 'Barang mutasi berhasil dibuat',
      life: 3000,
    });
    if (!stayModel.value) {
      backRoute();
...
watch(
  ...
  () => {
    if (
      isWarehouseModel.value === 'store' ||
      isWarehouseModel.value === 'warehouse'
    ) {
      productModel.value = [];
      quantityModel.value = [];
      quantitySelected.value = [];
...
watch(
  () => fromStoreModel.value,
  () => {
    if (fromStoreModel.value) {
      productModel.value = [];
      quantityModel.value = [];
      quantitySelected.value = [];
      totalProductFields.value = 0;
...
<template>
  <Card>
    <form class="flex flex-col p-4 gap-3" @submit.prevent="createStockMutation">
