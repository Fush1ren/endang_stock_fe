...
import type { StockInReport } from '@/types/stock.type';
...
// const transactionFilter = ref<Date>();
const stockInReport = ref<StockInReport[]>([]);
...
const tableColumns: TableColumn[] = [
  {
    ...
    sortable: false,
    bodyTemplate: (rowData: StockInReport) => {
  ...
  },
  {
    ...
    preset: {
      type: 'multirow',
      fieldAttributes: (data: StockInReport): MultiRowAttribute[] => {
...
const triggerFilter = (): void => {
  ...
  if (transactionFilter.value && transactionFilter?.value?.length > 0) {
    ...
    eventBus.emit('data-table:apply-filter', {
      tableName: 'table-report-product-in',
      filter: parsedFilter,
    });
  }
...
const togglePrint = (): void => {
  ...
  doc.text('Laporan Barang Masuk', 105, 10, {
    align: 'center',
  });
  ...
  let groupedMap = [];
  for (const trx of stockInReport.value) {
...
const tableData = async (
  params: QueryParams,
): Promise<FetchListResponse<StockInReport> | undefined> => {
  try {
    const { data } = await StockService.getStockInReport(params);
    stockInReport.value = data?.data?.data || [];
    return data;
  } catch (error) {
  ...
  <Card>
    <div class="flex flex-col gap-3 text-black">
      <div class="flex flex-row gap-1 w-fit">
        <span class="text-lg font-semibold">Laporan Barang Masuk</span>
      </div>
      <div class="flex flex-col gap-2 pt-2">
        ...
        <DataTable
          ...
          :use-options="false"
          table-name="table-report-product-in"
