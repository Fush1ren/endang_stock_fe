...
import type { StockOutReport } from '@/types/stock.type';
...
const stockOutReport = ref<StockOutReport[]>([]);
...
const tableColumns: TableColumn[] = [
  {
    ...
    sortable: false,
    bodyTemplate: (rowData: StockOutReport) => {
  ...
  },
  {
    ...
    preset: {
      type: 'multirow',
      fieldAttributes: (data: StockOutReport): MultiRowAttribute[] => {
...
const triggerFilter = (): void => {
  ...
  if (transactionFilter.value && transactionFilter?.value?.length > 0) {
    ...
    eventBus.emit('data-table:apply-filter', {
      tableName: 'table-report-product-out',
      filter: parsedFilter,
    });
  }
...
const togglePrint = (): void => {
  ...
  doc.text('Laporan Barang Keluar', 105, 10, {
    align: 'center',
  });
  ...
  for (const trx of stockOutReport.value) {
...
const tableData = async (
  params: QueryParams,
): Promise<FetchListResponse<StockOutReport> | undefined> => {
  try {
    const { data } = await StockService.getStockOutReport(params);
    stockOutReport.value = data?.data?.data || [];
    return data;
  } catch (error) {
  ...
  <Card>
    <div class="flex flex-col gap-3 text-black">
      <div class="flex flex-row gap-1 w-fit">
        <span class="text-lg font-semibold">Laporan Barang Keluar</span>
      </div>
      <div class="flex flex-col gap-2 pt-2">
        ...
        <DataTable
          ...
          :use-options="false"
          table-name="table-report-product-out"
